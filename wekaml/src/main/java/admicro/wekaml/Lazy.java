...
public class Lazy {
	...
	public List<List<String>> readDataSVC(String dataPath) throws IOException {
		List<List<String>> resultTopK = new ArrayList<>();
		try (Reader reader = Files.newBufferedReader(Paths.get(dataPath));
				CSVReader csvReader = new CSVReader(reader);) {
	 ...
	 * @throws IOException
	 */
	public void writeDataSVC(String resultPath, List<String> listTailName,
			List<HashMap<String, List<List<String>>>> listAlgorithm) throws IOException {
		File file = new File(resultPath);
		file.getParentFile().mkdirs();
		try (Writer writer = Files.newBufferedWriter(Paths.get(resultPath));
			...
			for (int algoIndex = 0; algoIndex < listAlgorithm.size(); algoIndex++) {
				...
				for (String key : listTailName) {
					...
					for (int i = 0; i < topK.size(); i++) {
						...
						recordHeader2.add("Feature");
						recordHeader2.add("TP");
						recordHeader2.add("FP");
						recordHeader2.add("FN");
						recordHeader2.add("TN");
						recordHeader2.add("Precision");
						recordHeader2.add("Recall");
						recordHeader2.add("F1");
						recordHeader2.add("Accuracy");
						recordHeader2.add("");
...
//		listDatasetName.add("dataset3");
		...
		// Add feature name		
//		listFeature.add("keyword");//3
//		listFeature.add("timeSpan");//15
//		listFeature.add("minSim");//13
//		listFeature.add("maxSim");//12
//		listFeature.add("avgSim");//10
//		listFeature.add("jaccardTitle");//6
//		listFeature.add("ib");//9
//		listFeature.add("sumOfMax");//11
//		listFeature.add("lm");//8
//		listFeature.add("bm25");//7
//		listFeature.add("jaccardBody");//5
//		listFeature.add("jaccardSim");//14
//		listFeature.add("cosineTF");//4
//		listFeature.add("LDASim");//16
//		listFeature.add("TFIDF"); //17
		
		listFeature.add("14_Features");
		listFeature.add("13_Features");
		listFeature.add("12_Features");
		listFeature.add("11_Features");
		listFeature.add("10_Features");
		listFeature.add("9_Features");
		listFeature.add("8_Features");
		listFeature.add("7_Features");
		listFeature.add("6_Features");
		listFeature.add("5_Features");
		listFeature.add("4_Features");
		listFeature.add("3_Features");
		listFeature.add("2_Features");
		listFeature.add("1_Feature");		
...
//		HashMap<String, List<List<String>>> tfidfResult = new HashMap<>();
		...
		for (String dataset : listDatasetName)
			for (String criteria : listCriteria) {
				String svmPath = "C:/Users/ADMIN/Desktop/Demo/auto_fill_data/30_11_2018_3/leave_one_feature_out/Positive/"
...
//						listTailName, reptreeResult, m5pResult, gbrtResult, tfidfResult);
				// So sanh theo feature
				writeData(
						"C:/Users/ADMIN/Desktop/Demo/auto_fill_data/30_11_2018_3/leave_one_feature_out/Positive/"
	...
	public void mapDataSvc() throws IOException {
		List<String> listDatasetName = new ArrayList<>();
		List<String> listCriteria = new ArrayList<>();
		List<String> listFeature = new ArrayList<>();
		List<String> listTailName = new ArrayList<>();
...
//		listDatasetName.add("dataset3");
		...
		HashMap<String, List<List<String>>> svcResult = new HashMap<>();

		for (String dataset : listDatasetName)
			for (String criteria : listCriteria) {
				String svcPath = "C:/Users/ADMIN/Desktop/Demo/auto_fill_data/26_11_2018/leave_one_feature_out/"
						+ dataset + "/" + criteria + "/SVC/";
				for (String feature : listFeature) {
					String name = "_" + dataset + "_" + criteria + "_" + feature;
					listTailName.add(name);
					
					List<List<String>> listSvc = readDataSVC(svcPath + name + "/eval_brief.txt");
					svcResult.put(name, listSvc);
				}
				Collections.sort(listTailName);
				List<HashMap<String, List<List<String>>>> listAlgorithm = new ArrayList<>();
				listAlgorithm.add(svcResult);
				// So sanh theo thuat toan
//				writeData(
//						"C:/Users/ADMIN/Desktop/Demo/auto_fill_data/24_10_2018/leave_one_feature_out/Positive/"
//								+ dataset + "/" + dataset + "_" + criteria + ".csv",
//						listTailName, reptreeResult, m5pResult, gbrtResult, tfidfResult);
				// So sanh theo feature
				writeDataSVC("C:/Users/ADMIN/Desktop/Demo/auto_fill_data/26_11_2018/leave_one_feature_out/"
								+ dataset + "/" + dataset + "_feature_" + criteria + ".csv",
						listTailName, listAlgorithm);
...
//		HashMap<String, List<List<String>>> gbrtResult = new HashMap<>();
		...
		for(String dataset:listDatasetName)
			for(String criteria:listCriteria)
			{
				String svmPath = "C:/Users/ADMIN/Desktop/Demo/auto_fill_data/26_11_2018/all_features/Positive/"
						+ dataset + "/" + criteria + "/SVM/";
...
//						+ dataset + "/" + criteria + "/GBRT/";
				String tfidfPath = "C:/Users/ADMIN/Desktop/Demo/auto_fill_data/26_11_2018/all_features/Positive/"
						+ dataset + "/" + criteria + "/TFIDF/";
...
//						listTailName, reptreeResult, m5pResult, gbrtResult, tfidfResult);
				writeData(
						"C:/Users/ADMIN/Desktop/Demo/auto_fill_data/26_11_2018/all_features/Positive/" + dataset
								+ "/" + dataset + "_" + criteria + ".csv",
